...
document.addEventListener("DOMContentLoaded",function(){const e=new MouseFollower({container:"body",speed:.55});const t=document.querySelectorAll(".footer-links li, .burger-lines-wrap, .menu_open-social");var n;function i(t){t.addEventListener("mouseenter",()=>{e.hide()}),t.addEventListener("mouseleave",()=>{e.show()})}n="-inverse-scaled",t.forEach(function(t){t.addEventListener("mouseenter",()=>{e.addState(n)}),t.addEventListener("mouseleave",()=>{e.removeState(n)})});const o=document.querySelector(".btn-submit"),s=document.querySelector(".tg_link");i(o),i(s);const c=document.querySelector(".name_input"),a=document.querySelector(".email_input"),r=document.querySelector(".text_input"),l=document.querySelector(".last-row-br");function d(e,t){let n=e,i=window.getSelection(),o=document.createRange();e.addEventListener("click",function(e){e.target.classList.contains("active")||(o.setStart(n,0),o.setEnd(n,0),i.removeAllRanges(),i.addRange(o))}),e.addEventListener("focusout",function(e){const n=e.target;n.innerHTML.length<1&&(n.style.color="#d8d8d8",n.innerHTML=t,n.classList.remove("active"),n.classList.contains("text_input")&&(l.style.display="inline"))}),e.addEventListener("keyup",function(e){const t=e.target;t.classList.contains("active")||(t.style.color="transparent",t.innerText=t.innerText.slice(0,1),o.setStart(n,1),o.setEnd(n,1),i.removeAllRanges(),i.addRange(o),t.classList.contains("text_input")&&(l.style.display="none"),t.classList.add("active"))})}d(c,"Rudolf"),d(a,"r@email.com"),d(r,"the new website development for my company");const u=document.querySelector(".span-br");c.addEventListener("input",function(e){const t=window.innerWidth;let n;t>1800?n=212:t>1366?n=212:t>1023&&(n=154);const i=e.target.offsetWidth;r.offsetWidth;t>768&&(i>n?(u.style.display="none",l.style.display="none"):u.style.display="inline")});const y=document.querySelector(".hello"),m=document.querySelector(".feedback_main");function p(e){if(e.innerHTML.length>1&&e.classList.contains("active"))return!0}function f(e){if(e.innerHTML.length>5&&e.innerHTML.includes("@")&&e.classList.contains("active"))return!0}function L(e){if(e.innerHTML.length>5&&e.classList.contains("active"))return!0}o.addEventListener("click",function(){const e=c,t=a,n=r,i=document.querySelector(".contact_notice_wrap"),o=document.querySelector(".contact_notice_text--name"),s=document.querySelector(".contact_notice_text--email"),l=document.querySelector(".contact_notice_text--subj"),d=document.querySelectorAll(".contact_notice_text");i.classList.remove("active"),setTimeout(()=>{d.forEach(e=>{e.style.display="none"})},1e3),p(e)&&f(t)&&L(n)?async function(e="",t={}){const n=await fetch(e,{method:"POST",mode:"cors",cache:"no-cache",credentials:"same-origin",headers:{"Content-Type":"application/json"},redirect:"follow",referrerPolicy:"no-referrer",body:JSON.stringify(t)});return await n.json()}("/send.php",{name:e.innerText,email:t.innerText,text:n.innerText}).then(e=>{if(console.log(e),e.error)switch(e.error){case"all":[o,l,s].forEach(e=>{e.style.display="inline"});break;case"name_email":[o,s].forEach(e=>{e.style.display="inline"});break;case"name_text":[o,l].forEach(e=>{e.style.display="inline"});break;case"email_text":[l,s].forEach(e=>{e.style.display="inline"})}else{const e=document.querySelector(".contact_input.name_input.active").innerHTML;document.querySelector("#hello_name").innerHTML=e,function(e,t){e.classList.add("transparent"),setTimeout(()=>{e.style.display="none",t.style.display="block",setTimeout(()=>{t.classList.remove("transparent")},5)},500)}(m,y)}}):setTimeout(()=>{p(e)||(o.style.display="inline"),L(n)||(l.style.display="inline"),f(t)||(s.style.display="inline"),i.classList.add("active")},1e3)})});
